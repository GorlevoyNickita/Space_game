...
{
    //Enemy_3 расширяет класс Enemy Траектория движения  Enemy_3 вычисляется путем линейной интерполяции кривой Безье по более чем двум точкам
    ...
    // И снова метод  Start хорошо подходит для наших целей потому что не используется суперклассом  Enemy
    void Start()
    {
        points = new Vector3[3];// Инициализировать массив точек
        // Начальное позиция уже определена в Main.SpawnEnemy()
        points[0] = pos;
